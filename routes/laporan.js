...
router.get('/saldo-awal', async (req, res) => {
    ...
    try {
        ...
        const query = `
            ...
            FROM akun a
            LEFT JOIN transaksi t ON a.id = t.id_akun AND t.tanggal < ?
            WHERE a.id = ?
            GROUP BY a.id, a.saldo_awal;
        `;
        const [results] = await db.query(query, [tanggal, akunId]);

        if (results.length === 0) {
            // Jika tidak ada transaksi sama sekali, ambil saja saldo awal dari akun
            const [akunRows] = await db.query('SELECT saldo_awal FROM akun WHERE id = ?', [akunId]);
        ...
        res.status(200).json({ saldo_awal: parseFloat(results[0].saldo_awal_periode) });
    } catch (error) {
